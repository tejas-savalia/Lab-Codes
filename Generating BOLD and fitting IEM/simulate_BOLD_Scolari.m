% function simulate_BOLD_Scolari(voxNoiseRatio, NTFbaseline, NTFamp, NTFsig)
...
function [NTFresp] = splice_NTFresponses(NTFresp_Base, p, sub, Cnt)
    ...
    for orient = 1:p.nOrients
        if p.nOrients == 9
            startPoint = [1 21 41 61 81 101 121 141 161 ];
%             allPoints = zeros(1,180);
            ...
            %find which 90 elements in the 180 element vector need to be made negative
            findEndPoints = circshift(startPoint, -(orient-1));
            posEndPoints = [findEndPoints(1) ;  mean(findEndPoints(5:6))];
            if posEndPoints(1) < posEndPoints(2)
                posPoints = [posEndPoints(1): posEndPoints(2)];
%                 allPoints(posPoints)=1;                
            elseif posEndPoints(1) > posEndPoints(2)
                posPoints = [posEndPoints(1):180 1:posEndPoints(2)];
%                 allPoints(posPoints)=1;     
            elseif posEndPoints(1) == posEndPoints(2)
                posPoints = [posEndPoints(1): posEndPoints(1)+90]
%                 allPoints(posPoints)=1;
            end
            negPoints = ones(1,180);
            negPoints(posPoints) = 0;
            negPoints = find(negPoints==1);
            
            finalGainDist(posPoints) = gainDist(posPoints);
            finalGainDist(negPoints) = -(gainDist(negPoints)-1)+1;
...
%             gainDist = abs( p.gainFactor * -((mod((stim-p.NTFcenters+pi/2),pi)-pi/2)/p.NTFsig^2) .* exp(-(mod((stim-p.NTFcenters+pi/2),pi)-pi/2).^2 / (2*p.NTFsig^2)) ) + 1;
            ...
            NTFresp(orient,:) = NTFresp_Base(orient,:).*finalGainDist;
%             stop=1
        else
