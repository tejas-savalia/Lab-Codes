% function fit_IEM_Scolari(p, BOLD_data)
...
PosOffsetData = zeros(p.nSubs,p.nChans);
NegOffsetData = zeros(p.nSubs,p.nChans);
...
%     trn = p.data(sub).voxResp; %[nTrials nVox]
    ...
    if sub == 1
        meanPosOffset = mean(mean(p.data(sub).voxResp(p.g==1,:)));
        meanNegOffset =  mean(mean(p.data(sub).voxResp(p.g==2,:)));
    end
    
    for run = 1:p.nRuns/2 % just go through runs 1 - 4, then hold out run and run+nRuns/2, so that 2 runs held out each time
                            % note this ensures that 1 PosOffset and 1 NegOffset run are held out, but only if the data are ordered with all runs of one type in first half of runs
...
%         x = x - nanmean(x(:));% + nanmean(tst(:));
    ...
    %     PosOffsetMean(rCnt) = nanmean(nanmean(tst(tstg==1,:)));
    %     NegOffsetMean(rCnt) = nanmean(nanmean(tst(tstg==2,:)));
        ...
        predictedPosOffset = predictedBOLD(tstg==1,:);
        predictedNegOffset = predictedBOLD(tstg==2,:);
        fprintf('Subject %d\n', sub);
        mean_predPosOffset = mean(mean(predictedPosOffset));
        mean_predNegOffset = mean(mean(predictedNegOffset));
        mean_testBOLDPosOffset = mean(mean(tstData(tstg==1)));
        mean_testBOLDNegOffset = mean(mean(tstData(tstg==2)));
    ...
    PosOffsetVTF=(nanmean(C2_centered(task==1, :)));%;+nanmean(lowMean);
    NegOffsetVTF=(nanmean(C2_centered(task==2, :)));%+nanmean(highMean);

    PosOffsetData(sub,:) = PosOffsetVTF;
    NegOffsetData(sub,:) = NegOffsetVTF;
%     
%     C2_unshifted(sub,:,:) = C2;
%     W_unshifted(sub,:,:) = W;
...
if subtract_mean_chan == 0
    d(:,1) = mean(NegOffsetData)';    
    sem(:,1) = (std(NegOffsetData)./sqrt(size(NegOffsetData,1)))';
    
    d(:,2) = mean(PosOffsetData)';    
    sem(:,2) = (std(PosOffsetData)./sqrt(size(PosOffsetData,1)))';
    
elseif subtract_mean_chan ==1  %PLOT CHAN RESPS WITH MEAN SUBTRACTED

    NegOffsetData_mean_subtracted = NegOffsetData - mean(NegOffsetData(:)); 
    d(:,1) = mean(NegOffsetData_mean_subtracted)';  
    sem(:,1) = (std(NegOffsetData_mean_subtracted)./sqrt(size(NegOffsetData_mean_subtracted,1)))';

    PosOffsetData_mean_subtracted = PosOffsetData - mean(PosOffsetData(:));  
    d(:,2) = mean(PosOffsetData_mean_subtracted)';
    sem(:,2) = (std(PosOffsetData_mean_subtracted)./sqrt(size(PosOffsetData_mean_subtracted,1)))';
end
...
legend({'NegOffset Trials', 'PosOffset Trials'})
legend boxoff
...
% tmp = (fliplr(PosOffsetData(:,1:4))+PosOffsetData(:,6:9))./2;
% PosOffsetDataC = [PosOffsetData(:,5), tmp];% concatenate the central datapoint (5), with the 'folded' data from 1:4 and 6:9
% tmp = (fliplr(NegOffsetData(:,1:4))+ NegOffsetData(:,6:9))./2;
% NegOffsetDataC = [NegOffsetData(:,5), tmp];

tmp = (fliplr(NegOffsetData(:,1:4))+PosOffsetData(:,6:9))./2;
CuedDirectionDataC = [PosOffsetData(:,5), tmp];
tmp = (fliplr(PosOffsetData(:,1:4))+ NegOffsetData(:,6:9))./2;
UncuedDirectiontDataC = [NegOffsetData(:,5), tmp];
...
if subtract_mean_chan == 0    
    d(:,1) = mean(UncuedDirectiontDataC)';  %lowDataC is the folded data (two halves of the curve collapsed into one)
    sem(:,1) = (std(UncuedDirectiontDataC)./sqrt(size(UncuedDirectiontDataC,1)))';
   
    d(:,2) = mean(CuedDirectionDataC)';    
    sem(:,2) = (std(CuedDirectionDataC)./sqrt(size(CuedDirectionDataC,1)))';
elseif subtract_mean_chan ==1  %PLOT CHAN RESPS WITH MEAN SUBTRACTED
    d(:,1) = mean(UncuedDirectiontDataC)' - mean(UncuedDirectiontDataC(:));  
    sem(:,1) = (std(UncuedDirectiontDataC)./sqrt(size(UncuedDirectiontDataC,1)))';
    
    d(:,2) = mean(CuedDirectionDataC)' - mean(CuedDirectionDataC(:));
    sem(:,2) = (std(CuedDirectionDataC)./sqrt(size(CuedDirectionDataC,1)))';    
end
...
ylabel('Channel response (a.u.)')
legend({'Uncued Direction', 'Cued Direction'})
legend boxoff         
