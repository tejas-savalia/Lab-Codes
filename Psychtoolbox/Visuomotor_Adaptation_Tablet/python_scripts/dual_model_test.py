...
def fit_participant(participant, curvatures, num_fits):
    ...
    for fit_parts in range(num_fits):
        ...
        starting_points = np.array([[0.9, 0.3, 0.99, 0.01]])
        for initial_point in starting_points:
            if participant%4 == 0 or participant%4 == 1:      
                ...
                fits = scipy.optimize.basinhopping(residuals_sudden, x0 = [initial_point[0], initial_point[1], initial_point[2], initial_point[3]], minimizer_kwargs={'args': (640, np.nan_to_num(np.ravel(curvatures[participant][1:-1]), nan = np.nanmedian(curvatures[participant][1:-1])), train_indices), 'method':'Nelder-Mead'})
                ...
                fits = scipy.optimize.basinhopping(residuals_gradual, x0 = [initial_point[0], initial_point[1], initial_point[2], initial_point[3]], minimizer_kwargs={'args': (640, np.nan_to_num(np.ravel(curvatures[participant][1:-1]), nan = np.nanmedian(curvatures[participant][1:-1])), train_indices), 'method':'Nelder-Mead'})
                #if fits.fun < fit_V[participant][fit_parts]:
                Af = fits.x[0]#fit_Af[participant][fit_parts] = fits.x[0]
                Bf = fits.x[1]#fit_Bf[participant][fit_parts] = fits.x[1]
                As = fits.x[2]#fit_As[participant][fit_parts] = fits.x[2]
                ...
                #fit_success[participant][fit_parts] = fits.success
            print (participant, V)
    return Af, Bf, As, Bs, V, train_indices
...
for participant in range(60):
    ...
    mask[fits[participant][5]] = False
    mask_train = np.zeros(640, bool)
    mask_train[fits[participant][5]] = True
    if participant%4 == 0 or participant%4 == 1:
        y_pred = dual_model_sudden(640, fits[participant][0], fits[participant][1], fits[participant][2], fits[participant][3])[0]
        y_pred1 = dual_model_sudden(64, fits[participant][0], fits[participant][1], fits[participant][2], fits[participant][3])[0]
    else:
...
def plots(test_data, test_predictions, test_data1, test_predictions1):
    ...
    fig.suptitle('Performance on held out Data: Dual State Model')
    ...
    fig.suptitle('Performance on Transfer phase: Dual State Model')
...
#    paramlist = list(itertools.product(range(1000, 1060), range(12), range(64), range(1, 2)))
    ...
    with open('fit_dual_bound_test.pickle', 'wb') as f:
        pickle.dump(fits, f)
    f.close()
