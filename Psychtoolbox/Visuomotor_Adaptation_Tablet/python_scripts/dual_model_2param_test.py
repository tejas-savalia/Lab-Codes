...
def dual_model_sudden(num_trials, Bf, Bs):
    ...
    for trial in range(num_trials - 1):
        ...
        #print(errors[trial])
        fast_est[trial+1] = (1 - Bf)*fast_est[trial] + Bf*errors[trial]
        slow_est[trial+1] = (1 - Bs)*slow_est[trial] + Bs*errors[trial]
...
def dual_model_gradual(num_trials, Bf, Bs):
    errors = np.zeros((num_trials))
    fast_est = np.zeros((num_trials))
    slow_est = np.zeros((num_trials))
    rotation_est = np.zeros((num_trials))
...
def residuals_sudden(params, num_trials, data_errors, train_indices):
    model_errors = dual_model_sudden(num_trials, params[0], params[1])[0]
...
def residuals_gradual(params, num_trials, data_errors, train_indices):
    model_errors = dual_model_gradual(num_trials, params[0], params[1])[0]
    model_errors_train = np.take(model_errors, train_indices)
    data_errors_train = np.take(data_errors, train_indices)
    residual_error = np.sum(np.square(model_errors_train - data_errors_train))
    if params[0] < params[1]:
...
def fit_participant(participant, curvatures, num_fits):
    ...
    for fit_parts in range(num_fits):
        ...
        starting_points = np.array([[0.2, 0.01]])
        for initial_point in starting_points:
            if participant%4 == 0 or participant%4 == 1:      
                ...
                fits = scipy.optimize.basinhopping(residuals_sudden, x0 = [initial_point[0], initial_point[1]], minimizer_kwargs={'args': (640, np.nan_to_num(np.ravel(curvatures[participant][1:-1]), nan = np.nanmedian(curvatures[participant][1:-1])), train_indices), 'method':'Nelder-Mead'})

                #if fits.fun < fit_V[participant][fit_parts]:
                Bf = fits.x[0]#fit_Af[participant][fit_parts] = fits.x[0]
                Bs = fits.x[1]#fit_Bf[participant][fit_parts] = fits.x[1]
                ...
                fits = scipy.optimize.basinhopping(residuals_gradual, x0 = [initial_point[0], initial_point[1]], minimizer_kwargs={'args': (640, np.nan_to_num(np.ravel(curvatures[participant][1:-1]), nan = np.nanmedian(curvatures[participant][1:-1])), train_indices), 'method':'Nelder-Mead'})
                #if fits.fun < fit_V[participant][fit_parts]:
                Bf = fits.x[0]#fit_Af[participant][fit_parts] = fits.x[0]
                Bs = fits.x[1]#fit_Bf[participant][fit_parts] = fits.x[1]
                #As = fits.x[2]#fit_As[participant][fit_parts] = fits.x[2]
...
#    paramlist = list(itertools.product(range(1000, 1060), range(12), range(64), range(1, 2)))
    ...
    with open('fit_dual_bound_test_2param.pickle', 'wb') as f:
        pickle.dump(fits, f)
    f.close()
