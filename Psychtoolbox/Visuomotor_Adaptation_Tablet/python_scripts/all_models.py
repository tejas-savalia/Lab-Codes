...
def dual_eight_param_sudden(num_trials, Af, Bf, As, Bs, Aft, Bft, Ast, Bst):
    errors = np.zeros((num_trials))
    rotation = 1.0
    fast_est = np.zeros((num_trials))
    slow_est = np.zeros((num_trials))
    ...
    for trial in range(num_trials - 1):
        ...
        #print(errors[trial])
            fast_est[trial+1] = Aft*fast_est[trial] - Bft*errors[trial]
            slow_est[trial+1] = Ast*slow_est[trial] - Bst*errors[trial]
...
def dual_eight_param_gradual(num_trials, Af, Bf, As, Bs, Aft, Bft, Ast, Bst):
    errors = np.zeros((num_trials))
    fast_est = np.zeros((num_trials))
    slow_est = np.zeros((num_trials))
    rotation_est = np.zeros((num_trials))
...
def dual_residuals_sudden_alpha(params, num_trials, data_errors, train_indices):
    model_errors = dual_model_sudden(num_trials, params[0], params[1], params[2], params[3], params[4])[0]
    model_errors_train = np.take(model_errors, train_indices[train_indices < len(model_errors)])
    data_errors_train = np.take(data_errors, train_indices[train_indices < len(model_errors)])
...
def dual_residuals_gradual_alpha(params, num_trials, data_errors, train_indices):
    #model_errors = dual_model_gradual(num_trials, params[0], params[1], params[2], params[3])[0]
    model_errors = dual_model_gradual_avg(num_trials, params[0], params[1], params[2], params[3], params[4])[0]
    model_errors_train = np.take(model_errors, train_indices[train_indices < len(model_errors)])
    data_errors_train = np.take(data_errors, train_indices[train_indices < len(model_errors)])
    #residual_error = np.sum(np.square(model_errors_train - data_errors_train))
    residual_error = -2*sum(stat.norm.logpdf(data_errors_train, model_errors_train, params[5]))
...
def dual_eight_params_residuals_sudden(params, num_trials, data_errors, train_indices):
    model_errors = dual_eight_param_sudden(num_trials, params[0], params[1], params[2], params[3], params[4], params[5], params[6], params[7])[0]
    model_errors_train = np.take(model_errors, train_indices)
    data_errors_train = np.take(data_errors, train_indices)
    residual_error = -2*sum(stat.norm.logpdf(data_errors_train, model_errors_train, params[8]))
    #residual_error = np.sum(np.square(model_errors_train - data_errors_train))
    if params[0] > params[2] or params[4] > params[6]:
        residual_error = residual_error + 10000000
    if params[1] < params[3] or params[5] < params[7]:
        residual_error = residual_error + 10000000
...
def dual_eight_params_residuals_gradual(params, num_trials, data_errors, train_indices):
    model_errors = dual_eight_param_gradual(num_trials, params[0], params[1], params[2], params[3], params[4], params[5], params[6], params[7])[0]
    model_errors_train = np.take(model_errors, train_indices)
    data_errors_train = np.take(data_errors, train_indices)
    residual_error = -2*sum(stat.norm.logpdf(data_errors_train, model_errors_train, params[8]))
    #residual_error = np.sum(np.square(model_errors_train - data_errors_train))
...
def dual_alpha_test_fit(participant, curvatures, num_fit_trials, train_indices):
    ...
    for initial_point in starting_points:
        if participant%4 == 0 or participant%4 == 1:      
            fits = scipy.optimize.basinhopping(dual_residuals_sudden, x0 = [initial_point[0], initial_point[1], initial_point[2], initial_point[3], initial_point[4], initial_point[5]], minimizer_kwargs={'args': (num_fit_trials, np.nan_to_num(np.ravel(curvatures[participant][1:]), nan = np.nanmedian(curvatures[participant][1:])), train_indices), 'method':'Nelder-Mead'})
            ...
            fits = scipy.optimize.basinhopping(dual_residuals_gradual, x0 = [initial_point[0], initial_point[1], initial_point[2], initial_point[3], initial_point[4], initial_point[5]], minimizer_kwargs={'args': (num_fit_trials, np.nan_to_num(np.ravel(curvatures[participant][1:]), nan = np.nanmedian(curvatures[participant][1:])), train_indices), 'method':'Nelder-Mead'})
            Af = fits.x[0]
            Bf = fits.x[1]
            As = fits.x[2]
            Bs = fits.x[3]
...
def dual_eight_test_fit(participant, curvatures, num_fit_trials, train_indices):
    train_length = num_fit_trials - int(np.floor(num_fit_trials/10.0))
    
    #train_indices = np.random.choice(num_fit_trials, train_length, replace = False)
    starting_points = np.array([[0.9, 0.3, 0.99, 0.01,0.9, 0.3, 0.99, 0.01, 0.05]])
    for initial_point in starting_points:
        if participant%4 == 0 or participant%4 == 1:      
            fits = scipy.optimize.basinhopping(dual_eight_params_residuals_sudden, x0 = [initial_point[0], initial_point[1], initial_point[2], initial_point[3], initial_point[4], initial_point[5], initial_point[6], initial_point[7], initial_point[8]], minimizer_kwargs={'args': (num_fit_trials, np.nan_to_num(np.ravel(curvatures[participant][1:]), nan = np.nanmedian(curvatures[participant][1:])), train_indices), 'method':'Nelder-Mead'})
            ...
            fits = scipy.optimize.basinhopping(dual_eight_params_residuals_gradual, x0 = [initial_point[0], initial_point[1], initial_point[2], initial_point[3], initial_point[4], initial_point[5], initial_point[6], initial_point[7], initial_point[8]], minimizer_kwargs={'args': (num_fit_trials, np.nan_to_num(np.ravel(curvatures[participant][1:]), nan = np.nanmedian(curvatures[participant][1:])), train_indices), 'method':'Nelder-Mead'})
            Af = fits.x[0]
            Bf = fits.x[1]
            As = fits.x[2]
            Bs = fits.x[3]
        ...
        print (participant, V)
    return Af, Bf, As, Bs, Aft, Bft, Ast, Bst, V, epsilon, train_indices
...
def run_fits_dual_eight(curvatures, num_fit_trials, num_fits):
    train_indices = pickle.load(open('train_indices_704.pickle', 'rb'))
    pool = Pool()
    res = np.zeros(num_fits, dtype = object)
    for i in range(num_fits):
        ...
        for participant in range(60):
            c_obj[participant] = curvatures
        participant_args = [x for x in zip(range(60), c_obj[range(60)],  np.repeat(num_fit_trials, 60), train_indices[i])]
        res[i] = np.reshape(np.array(pool.starmap(dual_eight_test_fit, participant_args)), (60, 11))
        print ("Mean Res in dual: ", i, np.mean(res[i][:, -3]))
